...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		$(document).on("visibilitychange", () => {
			if (!document.hidden) showMessage("啊啦啊啦，你终于回来了呀～", 6000, 9);
		});
	}
	...
	function welcomeMessage() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			if (now > 6 && now <= 8) text = ["枣安枣安枣安!重要的事要对重要的人说三遍！", "要不要来杯星巴克提提神呀", "今天也要元气满满鸭！"];
			else if (now > 8 && now <= 11) text = "上午好！工作顺利呀，要多起来走动走动哦！";
			else if (now > 11 && now <= 14) text = "到中午啦，现在是午餐时间！为了我的糖醋里脊冲冲冲鸭！";
			else if (now > 14 && now <= 17) text = "午后很容易犯困呢，今天的运动目标完成了吗？";
			else if (now > 17 && now <= 19) text = "最美不过夕阳红～！窗外夕阳的景色很美丽呢，";
			else if (now > 19 && now <= 21) text = "晚上好，今天过得怎么样？";
			else if (now > 21 && now <= 23) text = ["已经这么晚了呀，早点休息吧，掰掰咯～", "深夜时要爱护眼睛呀！", "别忘记和喜欢的人说晚安哦"];
	...
	//显示消息，text-消息内容, timeout-设置延时, priority-优先级
	...
	function loadRandModel() {
		...
		$.ajax({
			...
			success: function(result) {
				...
				else showMessage("嘻嘻我的新衣服好看嘛？(*๓´╰╯`๓)", 4000, 10);
